...
1. 作业完成后直接交给村长（即班主任）， 作业提交格式为，`你的微信昵称名字-作业名.py `, 如`黑姑娘-炸金花游戏.py`,  作业不要打包或压缩，就是.py格式就行，文件名一定要符合命名要求。 
2. 第一次作业提交截止日期是开营后第5天， 第2个作业是第8天，过期再交的不再批改，按未交处理。
3. 作业禁止抄袭，抄袭一经发现，成绩0分。
